...
public class EditThreadController {
	...
	public String editThreadPage(@RequestParam("titleId") int titleId, HttpServletRequest request, Model model) {
		var titlePermissionList = titlePermissionTblDao.selectAdminFlgList(titleId,
				Session.getUser(request).getUserId());
		if (titlePermissionList.size() == 0 || titlePermissionList.get(0) == 0) {
			Session.setErrorMessage(request, "編集権限がないスレッドは編集できません。");
